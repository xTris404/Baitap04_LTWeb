...
## Chức năng bổ sung (Bài tập mở rộng)

### 1. Kích hoạt tài khoản bằng OTP qua email
- Đăng ký → tài khoản `active=0`, gửi OTP 6 số (hiệu lực 10 phút).
- Trang `/activate` nhập OTP để kích hoạt.
- Có nút **Gửi lại OTP**.
- Đăng nhập bị chặn nếu chưa kích hoạt.

### 2. Đăng nhập / Đăng xuất
- Login kiểm tra mật khẩu + trạng thái active.
...
### 4. Products (quan hệ n-1 với Category)
- CRUD admin: `/admin/products`, add/edit/delete (upload ảnh).
- Trang chủ: 10 sản phẩm mới nhất.
- `/product`: phân trang 6 sp/trang.
- `/product/detail?id=`: chi tiết sản phẩm.

### 5. SiteMesh 3 + Bootstrap 5
- Decorator `WEB-INF/decorators/main.jsp` dùng Bootstrap 5 CDN.
- Navbar theo role (admin thấy QL danh mục / sản phẩm).

### 6. Validation form
- Client: Bootstrap `needs-validation` + `assets/js/validate.js` (password match).
- Server: `ValidationUtil` (email, phone, username, password, OTP, number).

### Cấu hình SMTP (gửi OTP thật)
...
Với Gmail: bật 2FA → tạo **App Password**.

Nếu `mail.enabled=false` (mặc định), OTP được **in ra console Tomcat** để test không cần SMTP.

### Database
Chạy `sql/create_database.sql` hoặc để Hibernate `hbm2ddl.auto=update` tự tạo cột `active`, `otp_code`, `otp_expiry` và bảng `products`.
